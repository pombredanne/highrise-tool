...
tables=$(sqlite3 $dbfile 'select name from sqlite_master where type="table"')
for name in $tables
do
  sqlite3 -header -csv $dbfile "select * from $name" > "http/highrise-$name.csv"
done
set -- $tables
if [ $# = 0 ]
then
